...
\begin{inparaitem}[\color{black}\textbullet]
    ...
    % \item $Proj(\b{x})=(\b{x}\cdot\b{v})/(\b{v}\cdot\b{v})\cdot \b{v}$\\
...
\begin{inparaitem}[\color{black}\textbullet]
    \item Gaussian:
    $f(x) = \frac{1}{\sqrt{2\pi \sigma^2}} \exp(- \frac{1}{2} \frac{(x-\mu)^2}{\sigma^2})$\\ %, \mathcal{N}(x|\mu, \sigma)
    $f(x) = \frac{1}{\sqrt{(2\pi)^d\det\Sigma}} \exp(- \frac{1}{2} (x-\mu)^T \Sigma^{-1} (x-\mu))$\\ % \mathcal{N}(x|\mu, \Sigma)
